...
exports.handle = async products => {
  for (const product of products) {
    try {
      ...
      const sendAlert =
        (currentPrice !== product.price && (!targetPrice || (currentPrice && currentPrice <= targetPrice))) ||
      ...
      logger.info(
        `${product.name} (${product.id}) - ${currentPrice} ${updatedProduct.currency} - ${currentAvailability}`
      );
